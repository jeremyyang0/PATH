...
import { type MethodDescriptor, type MethodFileIndex, workspaceCodeIndexService } from '../../code-index';
...
export class MethodsDataProvider implements vscode.TreeDataProvider<TreeItem> {
    ...
    public async loadData(): Promise<void> {
        try {
            ...
            this.originalData = this.buildTreeData(workspacePath, data.methodFiles);
            this.applySearch();
            this.onDidChangeTreeDataEmitter.fire();
        } catch (error) {
    ...
    private buildTreeData(workspacePath: string, methodResults: MethodFileIndex[]): TreeItem[] {
    ...
    private buildDirectoryChildren(
        ...
        methodsByFile: Map<string, MethodDescriptor[]>
    ): TreeItem[] {
    ...
    private createMethodItem(filePath: string, parentRelativeParts: string[], method: MethodDescriptor): TreeItem {
