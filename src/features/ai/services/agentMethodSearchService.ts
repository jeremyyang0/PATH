...
import { PythonIndexService } from '../../../shared/indexing/pythonIndexService';
...
export class AgentMethodSearchService {
    ...
    public async getCandidates(): Promise<AgentMethodCandidate[]> {
        ...
        const parsed = await parser.parseAllFiles();
        const eleVariablesByFile = new Map<string, Map<string, string>>();
        for (const file of parsed.results) {
            eleVariablesByFile.set(path.normalize(file.filePath), new Map(file.eleVariables.map(variable => [variable.name, variable.desc])));
        }

        const nextCandidates: AgentMethodCandidate[] = [];
        const nextSources = new Map<string, string>();

        for (const result of parsed.methodResults) {
