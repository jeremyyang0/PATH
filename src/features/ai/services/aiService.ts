...
import { PythonIndexService as EleParser, type ParseResult } from '../../../shared/indexing/pythonIndexService';

// 负责 AI 代码生成主链路：解析步骤、检索方法树、调用模型并回填代码。
...
async function getMethodTreeData(): Promise<MethodCandidate[]> {
    ...
    for (const methodResult of result.methodResults || []) {
        for (const method of methodResult.methods) {
            // 从file_path构建codePath
            const filePath = methodResult.filePath;
