...
import { type ElementDescriptor, type ElementFileIndex, workspaceCodeIndexService } from '../../code-index';
...
export class EleTreeDataProvider implements vscode.TreeDataProvider<TreeItem> {
    ...
    public readonly dragAndDropController = new DragAndDropController<TreeItem>();
    ...
    public async loadData(): Promise<void> {
        try {
            ...
            this.originalData = this.buildTreeData(workspacePath, data.elementFiles);
            this.applySearch();
            this.onDidChangeTreeDataEmitter.fire();
        } catch (error) {
    ...
    private async parseEleFiles(workspacePath: string) {
        return workspaceCodeIndexService.build(workspacePath);
    }

    private buildTreeData(workspacePath: string, results: ElementFileIndex[]): TreeItem[] {
        const methodDir = path.join(workspacePath, 'method');
        if (!fs.existsSync(methodDir)) {
            return [];
        }

        const resultsByFile = new Map<string, ElementFileIndex[]>();
    ...
    private buildDirectoryChildren(
        ...
        resultsByFile: Map<string, ElementFileIndex[]>
    ): TreeItem[] {
    ...
    private createElementItemsForResult(result: ElementFileIndex, parentFullPath: string, parentCodePath: string): TreeItem[] {
        const rootItems = new Map<string, TreeItem>();
        const childItems: ElementDescriptor[] = [];
        const items: TreeItem[] = [];

        for (const eleVar of result.elements) {
